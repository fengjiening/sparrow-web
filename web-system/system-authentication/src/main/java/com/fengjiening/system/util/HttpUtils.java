...
public class HttpUtils {
	...
	public static String doGet(String url, Map<String, String> urlParams) throws IOException {
		...
		return getResponseAsStringgbk(conn);
	}
	...
	private static String getResponseAsStringgbk(HttpURLConnection conn) throws IOException {
		InputStream es = conn.getErrorStream();
		if (es == null) {
			return getStreamAsString(conn.getInputStream(), DEFAULT_CHARSET_GBK);
		} else {
			String msg = getStreamAsString(es, DEFAULT_CHARSET_GBK);
			if (isEmpty(msg)) {
				throw new IOException(conn.getResponseCode() + " : " + conn.getResponseMessage());
			} else {
				throw new IOException(msg);
	...
	public static String sendHttpPost(String url ,Map<String, String> urlParams) throws Exception{
		if (isEmpty(url)) {
			throw new IllegalArgumentException("The parameter 'url' can not be null or blank.");
		}

		String paprm  = buildQuery1(urlParams, DEFAULT_CHARSET_GBK);
		HttpURLConnection conn = getConnection(new URL(url), METHOD_POST);
		conn.setConnectTimeout(CONNECTTIMEOUT);
		conn.setReadTimeout(READTIMEOUT);
		conn.getOutputStream().write(paprm.getBytes(DEFAULT_CHARSET_GBK));
		return getResponseAsStringgbk(conn);
	}

	public static String buildQuery1(Map<String, String> params, String charset) throws UnsupportedEncodingException {
		if (params == null || params.isEmpty()) {
			return "";
		}
		StringBuilder sb = new StringBuilder();
